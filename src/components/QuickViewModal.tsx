...
const QuickViewModal = ({
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="w-[95vw] max-w-3xl border-0 p-0 overflow-hidden bg-white text-gray-900 [&>button]:hidden flex flex-col max-h-[90vh]">
        <div className="flex-1 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          <div className="grid grid-cols-1 md:grid-cols-2 min-h-[400px]">
            {/* Close Button - Positioned absolutely relative to the content */}
            <button
              onClick={onClose}
              className="absolute top-4 right-4 w-8 h-8 rounded-full bg-white/80 hover:bg-white backdrop-blur-sm transition-colors flex items-center justify-center z-50 shadow-sm"
            >
            ...
            <div className="p-3  md:p-6 flex flex-col h-full bg-white">
              ...
              {/* Price & Stock */}
              <div className="flex items-center justify-between pt-4 border-t border-gray-200 mb-6 mt-auto">
